...
class dragndrop{
    ...
    constructor(){ 
        ...
        //only the target element as tthe selected element 
         this.svg.addEventListener('click', (evt)=>{this.get_selected(evt);}, false);
        //
    ...
    end_drag(evt) {
        if (this.selected_element && this.coord){
            //set a new attribute to the selected element 
            this.selected_element.setAttribute('newx', this.coord.x);
            this.selected_element.setAttribute('newy', this.coord.y);
      }
        //
...
class dragndrop_group extends dragndrop {
   ...
   start_drag(evt){
       ...
       if (evt.target.classList.contains('draggable')) {
            ...
            //
            //Set the mouse position during the drag in relation to the screen 
            this. offset = this.get_mouse_position(evt);
            //
            //Including the transform property to the selected element 
   ...
   //
   //Removes the selected property to any other t_span and sets it to the selected one 
   get_selected(evt){
       //
       //if the selected tag is an ellipse
       if (evt.target.classList.contains('draggable')) {
           //
            //Get all the ellipses from the dome 
            const collection =document.querySelectorAll('ellipse');
       ...
       //
       //if the selected element is an line for the relation 
       else if(evt.target.classList.contains('s_line')){
           //
            //Get all the ellipses from the dome 
            const collection =document.querySelectorAll('line');
            //
            //loop through the collection and remove the attribute of selected 
            collection.forEach($e=>{
                //
            ...
            //Set the selected element to be dragged to be the one clicked  
            //{the target element to which the drag will occurs}
            this.selected_line= evt.target;
            selected_element.setAttribute('selected', true);
   
       }
       //
       //if the selected element a tspan for the column attributes
       else if(evt.target.classList.contains('text')){
            if (this.selected_group){
                //
                //Get the selected elements is an ellipse and a text tspan
                const collection =document.querySelectorAll('text');
                //
                //loop through the collection and remove the attribute of selected 
                collection.forEach($e=>{
                    //
                ...
                //Set the selected element to be dragged to be the one clicked  
                //{the target element to which the drag will occurs}
                this.selected_text= evt.target;
                console.log(this.selected_text);
                this.selected_text.setAttribute('selected', true); 
            }
       }
   }
   //Change the translate arguments from 0 to new position to allow 
